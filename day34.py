...
def just_digits() -> None:
    import re, os
    
    with open(file="python.csv", mode="r", encoding="utf-8", errors="strict") as f:
        file_data = f.read()
        
    fopen = open(file="python.csv", mode="r", encoding="utf-8", errors="strict")
    
    file_data2 = fopen.read()
    
    digits = re.findall("\d+", file_data)
    
    digits2 = [digitz for digitz in file_data if digitz.isdigit()]
    
    digits3 = []
    for digity in file_data:
        if digity.isdigit():
            digits3.append(digity)
